...
export function MenteeDashboardLayout() {
  ...
  const displayName =
    user?.username?.split("@")[0]?.replace(/\./g, " ") ?? "Mentee";
    ...
    <div className="flex h-screen bg-gray-50">
      <aside className="w-56 bg-[#1a1f37] text-white flex flex-col shrink-0">
        ...
        <nav className="flex-1 px-3 py-4">
          <NavLink to="/mentee-dashboard" end className={navClass}>
            <LayoutGrid className="w-5 h-5" />
            <span className="text-sm font-medium">Dashboard</span>
          </NavLink>
          <NavLink to="/mentee-dashboard/programs" className={navClass}>
            <FolderOpen className="w-5 h-5" />
            <span className="text-sm font-medium">Programs</span>
          </NavLink>
          <NavLink to="/mentee-dashboard/mentors" className={navClass}>
            <Users className="w-5 h-5" />
            <span className="text-sm font-medium">Mentors</span>
          </NavLink>
          <NavLink to="/mentee-dashboard/calendar" className={navClass}>
            <Calendar className="w-5 h-5" />
            <span className="text-sm font-medium">Calendar</span>
          </NavLink>
          <NavLink to="/mentee-dashboard/events" className={navClass}>
            <Bell className="w-5 h-5" />
            <span className="text-sm font-medium">Events</span>
          </NavLink>
          <NavLink to="/mentee-dashboard/opportunities" className={navClass}>
            <Sparkles className="w-5 h-5" />
            <span className="text-sm font-medium">Opportunities</span>
          </NavLink>
          <NavLink to="/mentee-dashboard/messages" className={navClass}>
            <MessageSquare className="w-5 h-5" />
            <span className="text-sm font-medium">Messages</span>
          </NavLink>
          <button
            type="button"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-400 hover:bg-[#252b45] hover:text-white transition-colors w-full"
          >
